...
def get_n_books(driver: 'selenium webdriver',
    ...
    for book_num in range(1, iter + 1):
        ...
        book = driver.find_element_by_xpath(f'//div[{book_num}][@class="ss_book_box"]//*[@class="ss_book_list"]//li/a')
        dictionary['title'].append(book.text)  # 태그내용(=도서명) title에 저장
...
# 장르(정답)_책 수_오늘 날짜.csv로 저장
today = time.strftime("%Y-%m-%d", time.localtime(time.time()))
data.to_csv(f'./crawling_data/novel_{pages * 25}_{today}.csv', index=False)
