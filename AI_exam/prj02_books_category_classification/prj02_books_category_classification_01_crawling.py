        ...
        for temp in temps:
            if temp.text == "책소개":
                ...
                dictionary["summary"].append(
                    temp.find_element_by_xpath(
                        './/following-sibling::div[@class="Ere_prod_mconts_R"]'
                    ).text
                )
                dictionary["title"].append(
                    driver.find_element_by_xpath('//a[@class="Ere_bo_title"]').text
                )
...
def get_n_books(
    driver: "selenium webdriver", iter: "iteration number", dictionary: "dict to save result"
):
    """페이지 단위 크롤링"""
    # 1 ~ 반복횟수 + 1(주로 25번) 반복
    ...
    for cid in [170]:  # 여기에 해당 카테고리의 cid 리스트 작성
        ...
        for page in range(start_page, pages + 1):
            ...
            if (page % 100 == 0 and page != 0) or page == pages:
                ...
                data.to_csv(
                    f"./crawling_data/cid-{cid}_{len(data)}_{start_page}-{page}_{today}.csv",
                    index=False,
                )
                dictionary = {"title": [], "summary": []}
                start_page = page + 1
