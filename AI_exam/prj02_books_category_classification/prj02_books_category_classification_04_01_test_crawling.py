...
import prj02_books_category_classification_01_crawling as crawling
from data_concat import cid_to_cat
...
for cid in cids:
    start_page = 444
    for i, page in enumerate(range(start_page, start_page + pages + 1)):
        ...
        data.loc[i : i + 24, "category"] = cid_to_cat[cid]
...
data_total.to_csv(
    f"./crawling_data/raw_test_{len(data)}_{today}.csv",
    index=False,
)
